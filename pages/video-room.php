...
// Get user's dialect (either from URL parameter or user's default)
$dialect_id = isset($_GET['dialect']) ? $_GET['dialect'] : null;

// If no dialect specified in URL, get user's dialect
if (!$dialect_id) {
    $stmt = $conn->prepare("
        SELECT dialect_id 
        FROM user_dialects 
    ...
    $dialect_id = $result['dialect_id'] ?? 1;
}

// Verify user has access to this dialect
$stmt = $conn->prepare("
    SELECT ud.dialect_id, d.name as dialect_name, u.username, u.email
    FROM user_dialects ud
    JOIN dialects d ON ud.dialect_id = d.id
    JOIN users u ON ud.user_id = u.id
    WHERE ud.user_id = ? AND ud.dialect_id = ?
");
$stmt->execute([$_SESSION['user_id'], $dialect_id]);
$user = $stmt->fetch(PDO::FETCH_ASSOC);

// If user doesn't have access to this dialect, redirect to dashboard
if (!$user) {
    header('Location: ' . BASE_URL . 'pages/dashboard.php');
    exit();
}

// Generate consistent room ID for this dialect
$roomId = "dialect_" . $dialect_id . "_" . date("Ymd");
?>
<!DOCTYPE html>
<html lang="en">
<head>
